...
! function(u) {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (res2) {
                                            ...
                                            res2.forEach((item) => {
                                                ...
                                                const wishContent = (item.message || '').replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;");
                                                listDiv = listDiv + `<div class="wish-box-item"><strong>${name}</strong><p>${wishContent}</p></div>`;
                                            });
                                            $('.wish-box').html(listDiv);
    ...
    $(document).on('click', '.calendar-button-custom-click', function(){
        ...
        // console.log('calendar-button', $(this).parent().find('.calendar-button .atcb-click').eq(0));
    	$(this).parent().find('.calendar-button .atcb-click').eq(0).click();
    });
